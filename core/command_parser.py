...
class CommandParser:
    ...
    def _extract_target(self, text: str, command_type: str) -> str:
        ...
        hostname_match = re.search(
            self.parameter_patterns["hostname"], text, re.IGNORECASE
        )
        if hostname_match:
        ...
        if len(words) > 2:
            ...
            target_words = [
                word for word in words[2:] if word.lower() not in skip_words
            ]
            if target_words:
    ...
    def _calculate_confidence(self, text: str, command_type: str) -> float:
        ...
        if re.search(self.parameter_patterns["file_path"], text) or re.search(
            self.parameter_patterns["url"], text
        ):
            confidence += 0.1
