    ...
    def search_entries(self, query: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        for entry in reversed(self.memory_data):
            if (
                ...
                or query_lower in entry.get("result", "").lower()
            ):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_entries = sum(
                1 for entry in self.memory_data if entry.get("success", False)
            )
            ...
            return {
                ...
                "success_rate": (
                    (successful_entries / total_entries * 100)
                    if total_entries > 0
                    else 0
                ),
                "agents": agents,
                "oldest_entry": (
                    self.memory_data[0]["timestamp"] if self.memory_data else None
                ),
                "newest_entry": (
                    self.memory_data[-1]["timestamp"] if self.memory_data else None
                ),
            }
        except Exception as e:
