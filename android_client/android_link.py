    ...
    def _handle_command(self, client_id: str, message: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self.command_history.append(
                {"client_id": client_id, "command": command, "timestamp": time.time()}
            )
            ...
            if self.command_handler:
                ...
                self._send_to_client(
                    self.android_clients[client_id]["socket"], response
                )
            else:
    ...
    def _get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "uptime": time.time()
                - (
                    min([c["connected_at"] for c in self.android_clients.values()])
                    if self.android_clients
                    else time.time()
                ),
                "timestamp": time.time(),
            }
    ...
    def cleanup_inactive_clients(self, timeout: int = 300) -> None:
        ...
        try:
            ...
            for client_id in inactive_clients:
                ...
                self._remove_client(
                    client_id, self.android_clients[client_id]["socket"]
                )
