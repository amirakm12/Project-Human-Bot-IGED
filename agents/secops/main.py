...
class SecOpsAgent:
    ...
    def _security_scan(self, target: str) -> str:
        ...
        try:
            ...
            # Save results
            report_file = (
                self.output_dir / f"security_scan_{scan_target.replace('.', '_')}.txt"
            )
            with open(report_file, "w") as f:
                f.write("\n".join(results))

            return (
                f"✅ Security scan complete. Results saved to {report_file}\n\n"
                + "\n".join(results)
            )
    ...
    def _extract_url(self, text: str) -> str:
        ...
        domain_pattern = (
            r"(?:scan|check|test)\s+(?:the\s+)?([a-zA-Z0-9.-]+\.[a-zA-Z]{2,})"
        )
