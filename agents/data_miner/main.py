...
class DataMinerAgent:
    ...
    def _analyze_data(self, target: str) -> str:
        ...
        try:
            ...
            report_file = (
                self.output_dir / f"analysis_report_{Path(file_path).stem}.txt"
            )
            with open(report_file, "w") as f:
                f.write("\n".join(results))

            return (
                f"✅ Data analysis complete. Report saved to {report_file}\n\n"
                + "\n".join(results)
            )
        ...
        try:
            ...
            if file_path and Path(file_path).exists():
                ...
                if data is not None:
                    ...
                    # Save extracted data
                    output_file = (
                        self.output_dir / f"extracted_data_{Path(file_path).stem}.csv"
                    )
    ...
    def _visualize_data(self, target: str) -> str:
        ...
        try:
            ...
            if len(numerical_cols) > 0:
                ...
                axes[0, 0].hist(
                    data[col].dropna(), bins=30, alpha=0.7, edgecolor="black"
                )
        ...
        try:
            ...
            for col in categorical_cols:
                stats["categorical_stats"][col] = {
                    ...
                    "most_common": (
                        data[col].mode().iloc[0] if len(data[col].mode()) > 0 else None
                    ),
