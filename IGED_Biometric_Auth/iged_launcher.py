...
class SecureIGEDLauncher:
    ...
    def launch_iged(self) -> bool:
        ...
        try:
            ...
            self.iged_process = subprocess.Popen(
                [sys.executable, str(self.iged_path)], cwd=self.iged_path.parent
            )
    ...
    def show_gui_authentication(self) -> bool:
        ...
        progress = QProgressDialog(
            "Authenticating with Windows Hello...", "Cancel", 0, 0
        )
...
def main():
    ...
    if not BIOMETRIC_AVAILABLE:
        ...
        response = input(
            "Do you want to launch IGED without biometric authentication? (y/N): "
        )
        if response.lower() != "y":
