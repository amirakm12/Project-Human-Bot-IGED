...
def main():
    ...
    if not Path("venv").exists():
        print("❌ Virtual environment not found. Please run install_dependencies.py first.")
        return 1
    ...
    run_command("source venv/bin/activate && isort --profile=black --line-length=88 --skip=venv .")
    ...
    run_command("source venv/bin/activate && pip check > analysis_dependencies.log 2>&1")
    run_command("source venv/bin/activate && safety check > analysis_vulnerabilities.log 2>&1")
    
    # Generate clean requirements
    print("  → Generating clean requirements...")
    run_command("source venv/bin/activate && pipreqs . --force --encoding=utf-8 --savepath=requirements_clean.txt")
